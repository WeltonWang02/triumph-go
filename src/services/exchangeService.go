...
func getBestPrice(amount float64, symbol string, action string) (PriceInfo, error) {
	...
	for _, exchange := range exchanges {
		...
		if err != nil {
			return PriceInfo{}, fmt.Errorf("Error getting book data %s", err) // check the next source
		}
...
func getBookData(product string, exchange string) (OrderBook, error) {
	...
	if exchange == "coinbase" {
		...
		kSymbol := strings.Replace(product, "-", "", -1)
		url = "https://api.kraken.com/0/public/Depth?pair=" + kSymbol
	}
